...
async def process_game_data(raw_data: dict):
    ...
    try:
        # ETAPA 1 – Recebimento
        grid_logger.info(f"MOTOR: [RECEBIDO] Round {round_id} da fonte {source}")
        ...
        with database.get_db() as conn:
            last = conn.execute("SELECT grid_col, grid_row FROM rounds ORDER BY timestamp_captura DESC LIMIT 1").fetchone()
        
        if last and last['grid_col'] is not None:
            if last['grid_row'] < 5:
        ...
        grid_logger.info(f"MOTOR: [GRID] Posicionando Round {round_id} em Col:{clean_data['grid_col']} Lin:{clean_data['grid_row']}")

        # ETAPA 4 – Persistência
        was_inserted = database.insert_round(clean_data)
        
        if not was_inserted:
            grid_logger.warning(f"MOTOR: [DUPLICADO] Round {round_id} já processado.")
            return
        
        grid_logger.info(f"MOTOR: [OK] Round {round_id} salvo e sincronizado.")

        # ETAPA 6 – Inteligência
        stats = intelligence.calculate_stats(clean_data)
        grid_logger.info(f"MOTOR: [STATS] Inteligência atualizada.")
